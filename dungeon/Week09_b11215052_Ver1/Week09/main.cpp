...
public:
	void update(Hero& hero) {
		...
		if (canSee(this->sPos, hPos, dir)) {
			std::cout << "Creature has find the Hero in the ( " << dir.x << ", " << dir.y << " ) direction" << std::endl;
			this->sIcon = '!';
		}
		else {
...
int main(int argc, char** argv)
{
	...
	// let player decide to end the game or not
	bool endGameFlag = true;

	// Run the game loop
	while (!gKeyState[ValidInput::EESC] && endGameFlag) {
		...
		if (gHero.getHP() == 0) {
			...
			std::cout << "HP is empty, You died!\n Do you want to continue from save? (1) Yes (2) No: ";
			std::cin >> ck;
			...
			if (ck == 1) {
				...
				else if (gKeyState[ValidInput::EEnter]) {
					loadMap();
				}
			}
			else {
...
void saveMap(void) {
	...
	// read Hero data
	f << gHero.getPos();
	f << gHero.getHP() << " " << gHero.getLevel() << " " << gHero.getExp() << " " << gHero.getMaxExp() << " ";
